...
/**
 * A student's homework — SRS §20.2 (FR-HW-001, "Homework is available to the relevant
 * class/students") and §5's "access relevant to their own records". Read-only.
 *
 * ## Reachable all along, and linked from nowhere
 *
 * The `student` block in `permissions.js` grants `homework.view`, and `GET /homework` answers it:
 * `homework.service.js` narrows an account with a student profile to the homework set for its own
 * class, and to published rows only, so a draft a teacher has not released is never listed. The
 * student portal nevertheless offered no way in, and its footnote said the account showed published
 * exam results — as if that were all it could. This screen sends no class filter of its own; the
 * narrowing is the server's, and a second copy of it here could only disagree with it.
 *
 * ## Newest due date first
 *
 * The school's Homework screen keeps the API's default, `due_date` ascending, which by the end of a
 * term puts its first week on page one. A student opens this to see what is due now, so the list asks
 * for `due_date` descending: the latest work first, and what came before on the pages after.
 *
 * The parent surface has the same screen with one more column, saying which child a row is for
 * (`parent/homework/page.tsx`). No route in this app imports another route group's files, so the two
 * are separate pages for now; the table they share belongs in `components/` once it is extracted.
 */
 ...
 * `attachment_path`, plus `has_attachment`, plus the `subject` join. Only what this screen renders is
 * declared — the class is the student's own, so it is not a column here.
 */
interface Homework {
  id: number;
  title: string;
...
export default function StudentHomework() {
   ...
   * silence. `is_published` is not sent: the service forces it to true for a student, so a control
   ...
   * through the same `findById()` narrowing as the list, so a student can fetch the file of homework
        ...
        cell: (row) => (
          <div>
            ...
            {/*
              ...
              * homework — the instructions to work from — and a clamp that reveals the rest on hover
              * reveals nothing on a phone. The width is capped so a long one wraps inside its cell
  ...
  return (
    <div>
      <PageHeader
        title="Homework"
        description="Homework your school has published for your class, latest due date first."
      />
      ...
      {refusal ? (
        ...
        <EmptyNotice>
          {filtered ? 'No homework matches these filters.' : 'No homework has been published for your class yet.'}
        </EmptyNotice>
      ) : (
        <>
          <DataTable columns={columns} rows={rows} rowKey={(row) => row.id} caption="Your homework"
            busy={loading}
          />
