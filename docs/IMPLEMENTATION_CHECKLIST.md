...
`scripts/check-models.js`. As of **2026-09-10 (end of session 28)** they total **5,573 assertions, 0 failures,
0 skips**, every script exit 0, measured as one loop against live MariaDB:

| Script | Checks |
|---|---|
| `verify-addons.js` | 176 |
| `verify-ai.js` | 190 |
| `verify-app.js` | 205 |
| `verify-assignments.js` | 198 |
| `verify-attendance.js` | 83 |
| `verify-auth-chain.js` | 84 |
| `verify-auth-module.js` | 237 |
| `verify-billing.js` | 261 |
| `verify-concurrency.js` | 19 |
| `verify-deploy.js` | 67 |
| `verify-documents.js` | 136 |
...
| `verify-reports.js` | 115 |
| `verify-school-setup.js` | 168 |
| `verify-security.js` | 28 |
| `verify-seed.js` | 23 |
| `verify-staff.js` | 89 |
| `verify-students.js` | 158 |
| `verify-subscriptions.js` | 222 |
| `verify-teachers.js` | 87 |
| `verify-timetable.js` | 116 |
| `verify-users-roles.js` | 247 |
| `verify-validate.js` | 35 |
| **Total** | **5,573** |

Regenerated from `backend/tests/baseline.json`, the manifest `npm test` checks each run
against — last by script at the end of session 28, when fifteen rows had moved (5,495 → 5,573).
...
contradicted; those were corrected too, and `tests/verify.test.js` now fails the loop if one drifts.
...
`verify-plans.js` — **178 checks**: the eleven Joi
schemas directly (41), the route table by name (9), the service called directly (7), and 118 over real
HTTP against the real database — and by `verify-addons.js`, **176 checks**; see FR-SUB-009 in 3.F for what
...
| ID | Requirement | Status | Notes |
|---|---|---|---|
| FR-SUB-001 | Create plan (Name, Code, Description, Status, Public/Private, Recommended, Display Order) | Completed | `POST /plans`, `plans.manage`, platform-only, logged `create`. Table is `subscription_plans`, not `plans`. **Six** of the seven source fields are accepted; the seventh, Status, is **refused** — this row used to say "all seven", which its own next sentence contradicted, and then said Status was *stripped*, which it no longer is. `code` is upper-cased and a repeat is 409 `PLAN_CODE_TAKEN`. **A new plan is born `inactive`** — the column defaults to `active`, so the service overrides it — and a body naming `status` is now a 422 naming the field, on create, update and duplicate alike. Triage finding 11 is why: the outcome was always right, but a caller sending the seven fields §10.2 names got a 201 whose Status disagreed with their body and no way to tell, which is the shape `coupons.validation.js:60-62` argues against in this same codebase. **What this does not settle, and finding 11 keeps open:** whether Status should be *accepted* on create at all. §10.2:385 lists it among the submitted fields but never enumerates its legal values, and never says whether a plan may be born `active` while holding no `plan_prices` row — both would have to be answered before the field could be honoured. A plan created active would appear in the catalogue holding no price and no limits |
...
| §33 | Feature overrides, custom limits, custom pricing | Completed | `POST /:id/overrides` and `POST /:id/overrides/:overrideId/revoke`, `requirePlatformScope()` + **`subscriptions.overrides.manage`** — its own key, seeded to `super_admin` alone, because an override is the highest-precedence source in the entitlement chain and a school able to write one could grant itself any module, feature or limit. The entitlement resolver's deliberate skipping of `price` overrides is unchanged and still asserted in `verify-entitlement.js`. **§33's “Custom Pricing” is delivered by §10.4's Custom Price model, not by this override** — triage finding 39. `PRICING_MODELS.CUSTOM` is one of §10.4's five and `computeCycleAmount()` evaluates it from `plan_prices.custom_amount`, which is what makes a school-specific price billable. An `override_type: 'price'` row, by contrast, is **accepted with 201, stored, echoed back on the subscription detail, and read by nothing**: `grep -rn SubscriptionOverride src/modules/invoices/` returns nothing and `cycle_amount` is only ever `computeCycleAmount(price, quantity)`. It is inert, not deferred — `entitlementService.js` used to say it “belongs to invoice generation”, naming a consumer that does not exist, and that line is now corrected. Wiring it needs rules §33:1635 and §29:1426 never state (what it changes, from which cycle, how it meets §12.3/§12.4 proration and the first-cycle setup fee), and §35:1672 puts *“Additional pricing rules”* among the gaps the SRS declines to fill; refusing the type at validation would equally be a decision, since §33 names the capability. |
...
mounted below the tenant boundary, covered by `scripts/verify-billing.js` → **261 / 261, exit 0**,
...
| FR-BILL-003 | Manual payment submission (txn id + screenshot → Pending) | Completed | `POST /payments` (multipart, field `screenshot`), `payments.submit`, no platform scope. **Both halves of the title are optional** — the screenshot file and the transaction id alike. The file's optionality was always disclosed here; the transaction id's was not, and triage finding 52 is that omission. FR-BILL-003's Functional Behavior says *“School enters a transaction ID”* as a step, exactly as it says the school uploads a screenshot, and the source states no requiredness and no per-method rule — so whether cash and cheque submissions must carry one is a decision the SRS does not supply and finding 52 keeps open. What is recorded here is the behaviour: neither field is demanded. Status is always `pending` and settles nothing |
...
| §33 (§29 for quotations) | Refunds, taxes, quotations, overage, custom pricing, custom limits, feature overrides | Completed | **Write APIs now exist** for refunds (`POST /payments/:id/refunds`), taxes and quotations. Overage / custom limits / feature overrides remain 3.G. Custom pricing override is still skipped by the entitlement resolver on purpose, and is inert beyond that — stored and read by nothing; §33's “Custom Pricing” is met by §10.4's Custom Price model instead. See the §33 overrides row above and triage finding 39. **Quotations are not a §33 item.** `grep -i quotation docs/SRS-extracted.md` returns exactly one line in 1,698 — the bare table name `quotations` at :1445, inside §29’s Billing group — and §33’s SaaS Engine list (:1617-1637) is 21 named items with no Quotations among them. The module’s own files cited "§33 *Quotations*" in five places and now cite the §29 table alone, in the same form `roles.validation.js:12` already uses for the Roles screen. Whether a bare table name warrants a five-state lifecycle with automatic invoice issuance is a §35 "Additional workflows" question the SRS does not answer; correcting the citation does not settle it |
| §13.2 | Wallet | Completed | **Session 28, owner decision D5 — refunds credit, invoices spend.** A refund with `destination = wallet` credits `subscriptions.wallet_balance` (no gateway call); a Wallet payment is refused on submission when the balance is short and debits it when the payment is approved, under a row lock (`payments.service.js` `lockWallet`, `assertWalletCovers`, `debitWallet`, `creditWallet`). Asserted in `verify-billing.js`. No new table. The balance is shown on the subscription screen. (Known Issue #19 recorded the gap until then.) |
...
|---|---|---|---|
| FR-SCHOOL-001 | School settings (Logo, Name, Address, Phone, Email, Website, Favicon, Theme, Currency, Timezone) | Completed | All ten §14.1 fields are `school_settings` columns and all ten are accepted by `PATCH /school-settings`. `GET` find-or-**virtuals** and deliberately does not insert (asserted: the table is still empty after a GET); the first `PATCH` inserts, the second updates — both branches now exercised. `schools.name` is asserted not to be overwritten by `school_settings.name`. **An open reading, found by session 28's audit of the newer screens:** the Expected Outcome says the settings are "saved and applied within the school's tenant scope". *Saved* and *scoped* hold (FR-TENANT-004). *Applied*, if it means the product uses them, does not: nothing outside the settings module reads the display name, logo, theme, currency or timezone, and the settings screen now says so plainly instead of promising it. Which reading is meant is a question for the owner, not a gap to fill by guessing where each value should take effect |
| FR-SCHOOL-002 | Academic session create / activate / close | Completed | Seven endpoints. Create is always `upcoming`; activate flips `is_current` for the school inside a transaction and does **not** auto-close siblings; close stamps `closed` and clears current. **No DELETE** — close is the operation §14.2 names, and its absence is asserted. A closed session refuses edit and re-activation (`SESSION_CLOSED`). The `active → closed` transition is asserted in the audit row's `old_values`/`new_values` |
| FR-SCHOOL-003 | Classes, sections, class teachers, subjects | Completed | Nine endpoints. Unique per `(school_id, academic_session_id, name)`; classes may be created against an *upcoming* session; class teachers resolved through `loadTeacherInSchool()` and refused across schools. DELETE refuses on two grounds — enrolled students (`CLASS_HAS_STUDENTS`, FK is SET NULL) and cascade dependants (`CLASS_IN_USE` / `SECTION_IN_USE`, 8 and 6 tables), the latter added in session 16 after the cascade was found removing rows below the application |
| FR-SCHOOL-004 | Subject creation + class assignment + teacher assignment | Completed | Eleven endpoints. Assignment de-duplication uses a locking read inside a transaction, because `class_subjects_unique` and `teacher_subjects_unique` both contain nullable columns and **MySQL treats NULL as distinct in a UNIQUE index** — the index cannot enforce a whole-class assignment. Naming `teacher_id` on a class-subject upserts the matching `teacher_subjects` row. `DELETE /subjects/:id` is not a §14.4 behaviour but was already shipped; it is now guarded with `SUBJECT_IN_USE` rather than cascading into `exam_subjects` and, through it, `marks` |
...
| FR-TEACHER-001 | Teacher profile (qualification, joining date, subjects, classes) | **Completed** | `src/modules/teachers/`, six endpoints, `verify-teachers.js` → **87 / 87, exit 0**. Profile CRUD with every string width taken from the model; `GET /:id/assignments` reads `teacher_subjects` plus both senses of "classes". Assignment *writes* stay in §14.4's `POST /subjects/:id/teachers` so one relation has one writer. No DELETE — §15.3 names none; a teacher who leaves is deactivated by a `PATCH` |
| FR-TEACHER-002 | Teacher dashboard | **Completed** | `GET /teachers/dashboard`, resolved from `req.user.id` so no teacher can read a colleague's by changing a path id. A `teacher`-role user with no linked row gets 404 `TEACHER_PROFILE_MISSING`; a principal gets 403, because the seeded grants give `teachers.dashboard.view` to `super_admin` and `teacher` only |
| FR-STAFF-001 | Staff management (receptionist, accountant, librarian, other) | **Completed** | `src/modules/staff/`, four endpoints, `verify-staff.js` → **89 / 89, exit 0**. A deliberate near-copy of `teachers/` — same guards, `staff_limit`, the four §15.4 categories taken from the `staff.category` enum rather than restated. **It carries the re-activation ceiling `teachers/` shipped without** (§5a defect 21) from the start, asserted as a cycle: create to the ceiling, refuse, deactivate, reuse the freed allowance, then fail to re-activate. No dashboard and no DELETE — §15.4 names neither, unlike §15.3 and §15.2 |

### 3.K Attendance — §16

| ID | Requirement | Status | Notes |
|---|---|---|---|
| FR-ATT-001 | Mark student attendance (Present/Absent/Leave/Late) | Completed | `POST /attendance/students`, marked by a `teacher`-role user as the FR names. Bulk by shape; the unique index on `(student_id, attendance_date)` makes a re-mark a **correction** rather than a duplicate, which is why there is no PATCH and no DELETE. `verify-attendance.js` → **83 / 83, exit 0** |
...
| 4.4 | School: 17 MVP screens | §33 | Completed **The create screens are built too (session 26).** This row previously carried a warning that every "Add"/"New" button pointed at a route that did not exist — eighteen dead links, no create path through the UI at all. All eighteen now exist, each built against its module’s own create schema, with the field set and the required-ness taken from the API rather than chosen. `next build` generates 61 pages (was 43) and `verify-frontend.js` asserts **zero** unresolved internal links. Known Issue 30 is closed. **Four screens beyond §33's seventeen now exist, and the nav still carries exactly seventeen** — `school/settings` (FR-SCHOOL-001 and FR-SCHOOL-002, which had nine working endpoints and no caller at all: triage finding 18), `school/assignments` (§20.3), `school/ai` (§21) and `school/notifications` (§23). Each is reached from the Principal dashboard or a sibling screen rather than from the nav, which `verify-frontend.js` pins at §33's seventeen in both directions. That is the answer session 27 gave to twenty-four uncalled routes at once and it settles findings 18 and 60: §33 fixes two **screen lists**, and a screen list is not the requirement — FR-SCHOOL-001/002, FR-ASG-001 and FR-AI-001/002 are stated requirements with human actors, and were `Completed` against an API nothing could reach. The platform Settings screen remains the counter-example and still has no form, for the reason row 4.3 gives. `next build` generates **82** pages. |
...
**Session 28 found the last eight, and they were the other half of the same problem.** `Tested` means
*"not yet reachable in the running application — nothing mounts it"*, and it was the honest status for
FR-SCHOOL-001 through FR-SCHOOL-004, FR-STUDENT-001/002, FR-TEACHER-001/002, FR-STAFF-001 and
FR-SUB-008 when it was written. It is not now: **the uncalled-write-route queue closed in session 27 at
149 of 150**, so a route with no caller is the exception rather than the rule, and every one of these
eight is reached by a screen — verified one at a time against the tree rather than inferred from that
figure. `school/settings` calls `PATCH /school-settings` and all four session writes (FR-SCHOOL-001/002),
`school/subjects/[id]` calls the subject-teacher assignment (FR-SCHOOL-004), `(teacher)/teacher` calls
`GET /teachers/dashboard` (FR-TEACHER-002), and the class, student, teacher and staff screens cover the
rest. `verify-frontend.js` asserts five of them by method and path.

Two of the eight did **not** become `Completed`, because reachability was not the only thing wrong with
them: **FR-SUB-008** is `In Progress` — `enforceLimit` guards six of §11.2's eight keys, and its own note
already said "the machinery is verified, the coverage is not complete" while the status said otherwise;
and **FR-STUDENT-001** is `In Progress` because §15.1's "Documents" half has no code path at all
(`STUDENT_DOCUMENT` cites the FR in its own upload rule and has never had a caller), which is triage
finding 16 and is blocked on a document type and a permission the source never supplies. Both moved
*down* from `Tested`, which is what an accurate legend costs when it is applied rather than assumed.
Both reached `Completed` later in session 28, when the owner's decisions D2, D4 and D13
(`docs/OWNER-DECISIONS.md`) answered the questions that had blocked them — with FR-BILL-001 (D6)
and §13.2 Wallet (D5) beside them. The legend defines `Tested` as *"implemented and covered by a passing check,
...
at **5,573 assertions, 0 failures, 0 skips, every script exit 0** at the end of session 28
(2026-09-10), re-recorded into `backend/tests/baseline.json` in the same run (it read 5,495 before
the owner's decisions and the fixes after them). **This sentence previously claimed 5,512 assertions over
...
and 5,460 after the Known Issues #17 and #32 assertions. There is no history in this project, so where 5,512 came from is **unknown** and is not guessed
...
`scripts/verify-deploy.js` (67 assertions). **Nothing in it has been executed**: nginx, pm2,
