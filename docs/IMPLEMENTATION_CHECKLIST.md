...
mounted below the tenant boundary, covered by `scripts/verify-billing.js` → **257 / 257, exit 0**,
...
*down* from `Tested`, which is what an accurate legend costs when it is applied rather than assumed. The legend defines `Tested` as *"implemented and covered by a passing check,
