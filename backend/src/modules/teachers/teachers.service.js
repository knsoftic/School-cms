...
async function create(req, payload) {
  ...
  try {
    row = await db.sequelize.transaction(async (t) => {
       ...
       * not be measured against the ceiling — `verify-teachers.js` asserts exactly that, and an
      ...
      if (payload.is_active !== undefined ? payload.is_active : true) {
        await usageService.reserveHeadcount(school.id, LIMITS.TEACHER_LIMIT, 1, t);
      }
