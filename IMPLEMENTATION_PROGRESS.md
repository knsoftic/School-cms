...
process.
**5,440 checks pass, 0 fail, 0 skip, across thirty-nine scripts**, every script exit 0, measured as one
...
counting output lines undercounts the loop by 21 — a serial run prints 5,419 `PASS` lines for **5,440**
assertions.
...
table above is the session-28 figure: **5,440 / 0 FAIL / 0 SKIP / every script exit 0**, from a serial run.
...
- **The open rows of §5's Known Issues register.** #21 — the one with real consequences — is now
