...
process.
**5,484 checks pass, 0 fail, 0 skip, across thirty-nine scripts**, every script exit 0, measured as one
...
| `verify-students.js` | 147 | SRS §15.1 — the seven student endpoints, the admission ceiling, and the promotion / transfer / leaving machine |
...
counting output lines undercounts the loop by 21 — a serial run prints 5,463 `PASS` lines for **5,484**
assertions.
...
table above is the session-28 figure: **5,484 / 0 FAIL / 0 SKIP / every script exit 0**, from a serial run.
...
| `src/modules/addons/addons.service.js` | 495 | `list`/`findById`/`update`/`setActive`/`setPrices`, `readiness()`, `scopeFor()`, `planIdFor()` |
...
## 2an. Phase 4 — the uncalled-write-route queue, closed

`docs/VERIFICATION.md` sized a queue in session 26: **99 write routes with no caller in the UI**, the
largest single item in that document and the largest piece of outstanding frontend work. §2am closed
the biggest cluster. This section closes the rest.

Measured against the **generated OpenAPI document** — what the application actually mounts, rather
than what the route files declare — the figure moved:

| | mounted | with a caller | without |
|---|---|---|---|
| start of session 27 | 150 | 61 | 89 |
| after §2am (subscriptions) | 150 | 75 | 75 |
| **end of session 27** | **150** | **149** | **1** |

**The one route left is `POST /auth/refresh`**, and it is a false positive that is worth
understanding rather than fixing: `apiClient.ts:302` reaches it with a raw `fetch` because it *is*
the refresh mechanism — it cannot use the client that depends on it. The measurement therefore errs
in both directions, and both directions were found in practice this session.

### Nine clusters, and what each one meant in the product

- **exams (7)** — §19 was reachable at its two ends only: an exam could be created, and its results
  calculated and published. Everything between had no caller, so **FR-EXAM-002 could not be performed
  at all** and FR-EXAM-003 had nothing to calculate from. `school/exams/[id]` adds the papers and the
  marks; `school/exams/grade-scales` defines what an A is.
- **schools (6)** — the Schools screen was a directory. FR-SADMIN-003 through 007 all had routes and
  no callers, so a school could not be corrected, suspended, archived, deleted or given the Principal
  FR-SADMIN-007 requires.
- **payments (5), invoices (4), coupons (3)** — §13's money. Two of the five payment routes had in
  fact been called since the review screen was written; see the blind spot below.
- **add-ons (4)** — **every add-on in this product has zero prices**, which is why §2am's purchase
  form has to explain that a priceless add-on can only be granted at no charge. `PUT /:id/prices` is
  what fixes that, and it had never been callable.
- **parents (3)** — the starkest of the small ones. A parent account with no child linked signs in to
  an empty Parent dashboard, and **nothing in the product could link one**, so §15.2's portal was
  unreachable in practice however correct its code was.
- **users (2), roles (2)** — the access model. A user's permissions are two override arrays, not a
  list, and the editor says so; roles live on the same screen because §33 names Users and not Roles.
- **assignments (4), notifications (3), taxes (5), quotations (5), ai (6)** — the five clusters
  `VERIFICATION.md` filed under *needs a decision*. See below.
- **school settings (1), sessions (4)** — likewise.
- **eight single-route corrections** — organizations, classes (plus a delete), fee structures,
  homework, library books and both finance ledgers.

### The "needs a decision" column was one decision, and it is not a specification question

`VERIFICATION.md` split the 99 into 69 *buildable now* and 30 *needs a decision*, on the ground that
§33 names no screen for the latter. That split was right to draw and its second half rested on a
confusion worth naming: **§33 fixes two screen lists, and a screen list is not the requirement.**

- FR-SCHOOL-001 enumerates ten settings a school configures. FR-SCHOOL-002 requires that an academic
  session be created, activated and closed.
- §20.3 requires that work be set, handed in and marked.
- §23's `in_app` channel is defined as *the delivery itself* — a row is born `sent`, and reading it
  is the whole of what a recipient does with it.
- §13 has taxes and quotations, both with full CRUD, their own permission pairs and their own tables
  in §29's sixty-four.
- §21 describes a nine-step workflow in detail and twice says nothing beyond it is documented.

Every one of those was `Completed` or `Tested` on the strength of an API nothing could reach. So all
of them are built, and reached from a dashboard or a sibling screen rather than from a nav: both nav
lists are asserted against §33 in both directions by `verify-frontend.js`, and an eighteenth School
entry would be this product editing a list the source defines.

**The platform Settings screen remains the counter-example and still has no form.** There §33 names
the screen and nothing anywhere says what belongs on it — the cross-reference to §9 points at three
sections that define a dashboard, school management and principal creation. That screen states the
gap, and the distinction between it and the eight above is the whole of what "needs a decision" means.

### A blind spot found four more times, in code that already worked

`verify-frontend.js` collects `api.<method>(` followed **immediately** by a path literal. §2am hit
this and recorded it. Four more instances turned up here, in screens that had been calling their
routes correctly all along:

| screen | what it built | routes reporting as uncalled |
|---|---|---|
| payment review | `` `/payments/${id}/${decision}` `` | both FR-BILL-004 routes |
| add-on sale dialog | `` `/addons/${id}/${takingOff ? … }` `` | activate and deactivate |
| finance record dialog | `` `/finance/${kind}` `` | both §18 create routes |
| §2am's own first draft | `` `/subscriptions/${id}/${action}` `` | seven |

Eleven working routes, every one of them reported as having no caller, and the check that exists to
catch exactly that could not see any of them. All are written out now. **The lesson is not "avoid
interpolation"** — it is that a check reading source text is only as good as the shapes it knows, and
a measurement that disagrees with the code is worth resolving in whichever direction turns out to be
wrong.

### One shared component, and the split that justifies it

Seven screens needed the same thing at once — a `PATCH /:id` with no caller — so
`components/editDialog.tsx` owns the mechanism: seed from a row, send **only what changed**, put
`error.details` on the fields it names, banner the rest, close and reload. Each caller keeps its own
field set, its own hints and its own decisions about which fields exist.

That is the split `useRowAction` already draws and states: *the hook owns the state machine, the
caller owns the words*. A shared component that generated the copy would produce seven forms each
saying slightly the wrong thing, which is worse than seven copies of the right thing.

The one rule it carries beyond the mechanism is `nullable`, per field: a blank field is sent as
`null` when the column accepts one and **left out entirely** when it does not. Inferring that would
either silently drop a required field emptied by mistake, or refuse to clear one that should clear.

### What driving the screens found, again

Four defects, and no assertion could see any of them before it was written:

1. **Every list picker asked for more rows than the API allows.** `PAGINATION.MAX_LIMIT` is 100 and
   four screens asked for 200 or 500. That is a **422**, not a bigger page — `useCollection` reports
   the refusal as an error and the select renders as an **empty list**. The failure is silent in the
   worst way: the control looks loaded and offers nothing, which reads as "this school has no
   subjects" rather than as a bad request. Found because the exam papers screen offered no subject
   for a school that has one. `verify-frontend.js` now refuses any limit above the ceiling, read from
   `config/constants.js`, and the regression was proved by putting 200 back.
2. **A screen reading a column the API strips.** The user detail screen read
   `user.extra_permissions`, which is always `undefined`: `publicUser()` excludes both override
   arrays and `presentWithPermissions()` returns the picture nested under `permissions`.
   `verify-frontend.js` caught it — that rule exists for precisely this — and the screen now reads
   the nested block. The rule gained a one-file, two-field exemption for the request-body keys the
   permissions editor cannot avoid spelling, plus a second assertion holding it to writing them and
   never reading one back.
3. **A guard the security suite could not recognise.** The notification centre renders `action_url`,
   which is free text on the model, and guards it with `startsWith('/')` — internal paths only.
   `verify-security.js` demanded a `^https?:` allowlist and failed it. The guard is **strictly
   stronger**: `https://elsewhere.test` passes the allowlist and fails this. Requiring the weaker one
   would have forced the screen to widen what it accepts in order to satisfy a rule about not
   accepting too much, so the rule now takes either. Proved by deliberate regression.
4. **Two suite census figures went stale by being right.** The create-screen count moved 18 → 19 and
   `payments/new` had to adopt `FormActions`; the money-typing rule flagged three form-state
   interfaces. All three were the suite doing its job on new code, and all three were fixed in the
   code rather than in the rule.
5. **An assertion that shipped unfalsifiable, from the failure this project has now hit three
   times.** The exemption in finding 2 came with a companion check holding that screen to *writing*
   the two override keys and never reading one back — and its regex was authored through a heredoc,
   which turned `\b` into a **backspace byte**. The pattern on disk matched a literal control
   character, so it could never fire, and it passed while proving nothing. `npm test`'s
   stray-control-character rule is what caught it: `verify-frontend.js: 8`. Repaired with an editor
   rather than a generator, and then **proved by deliberate regression** — putting the stripped
   column back into the screen now fails it by name. The rule that caught this exists because of an
   earlier instance of the same mistake in an XSS assertion, and it has now earned its place twice.

### Verified in a browser, as the school principal

`PATCH /school-settings` saved a currency and a timezone and was read back from `school_settings`.
A paper was added to an exam through `POST /exams/:id/subjects`, a mark of 72 entered through
`POST /exams/marks`, and the paper submitted through `POST /exams/marks/submit` — after which the
service derived `outcome: 'pass'`, stamped `marks_submitted_at`, moved the exam to Marks Entry and
the screen locked the paper against further entry. `grade_name` stayed **null**, because the school
has no grade band: that is the two new screens agreeing rather than either being wrong, and it is
what `school/exams/grade-scales` exists to fix.

The four new school screens and the platform screens were each loaded and read back.

### Verification

- `tsc --noEmit` clean; `next build` clean, **71 pages** (was 63 at the start of the session).
- `verify-frontend.js` **204 → 280**: 66 new route assertions, the ceiling rule, and the
  permission-override exemption pair.
- `npm test` **5,629 tests, exit 0** — and it failed twice on the way there, once for the backspace
  above and once because `docs/IMPLEMENTATION_CHECKLIST.md` still quoted 204 for a suite the baseline
  recorded at 280. Both citations were re-derived from `tests/baseline.json`, which is the only place
  a per-suite figure should be read from.

### And the frontend had never been linted

`docs/VERIFICATION.md` recorded this and nothing had acted on it: Next 16 **removed `next lint`** and
`next build` no longer lints, `package.json`'s script still pointed at the removed command so
`npm run lint` failed outright, and there was **no eslint configuration of any kind**. Every screen
in this product was written after that upgrade, so none of them had ever been linted.

`frontend/eslint.config.mjs` now exists — flat config, `eslint-config-next/core-web-vitals`, which is
the base rules plus the Core Web Vitals ones raised from warnings to errors. The first run reported
**83 problems**, and seventeen of them were real:

- **Eight `<a href="/internal/path">`** where `<Link>` belongs, on Coupons, Plans, Schools,
  Subscriptions, Exams, Students, Subjects and Timetable. Each is a **full page reload** instead of a
  client navigation — which in this product also discards the in-memory access token, so the next
  screen begins by refreshing a session it need not have lost.
- **`window.location.href` in the school delete**, mine, with the same consequence. Now `router.push`.
- **Six `useMemo` column tables capturing a handler re-created every render.** Each was correct only
  by accident of what the handler happened to read; the handlers are `useCallback`-stable now, which
  makes the correctness structural.
- **A `useMemo` whose stated dependencies were narrower than what the React Compiler infers** — it
  read a whole `structure` and was told one property of it, so it could hold a stale list.
- **A ref read during effect cleanup** in the shell: focus returned to whatever the ref pointed at
  when the drawer closed rather than to the control that opened it. Captured at open now.

**One rule is turned off and named rather than left as a warning.**
`react-hooks/set-state-in-effect` fires 66 times, and every one is one of two patterns used
deliberately throughout: a fetch effect setting `loading` before awaiting, and a form re-seeding when
its row changes. The rule is right that both cost a render, and the alternatives are architectural —
a data-fetching library or Suspense for the first, remount-by-`key` for the second. Adopting either
is a deliberate decision about how this frontend fetches and should be made as one, not arrived at by
suppressing sixty-six findings a file at a time. Sixty-six permanent warnings would also make
`npm run lint` red for good, which is how a project stops running its linter. The config says all of
this and §7 carries the debt.

`verify-deploy.js` now asserts the frontend half the way it already asserted the backend's — config
present, script calling ESLint directly rather than the removed command, and the run itself green —
because a config nobody runs is how those seventeen come back. Regressed by turning one `<Link>` back
into an `<a>`: the suite fails and names it.
- `verify-security.js` **28**, with the link guard widened and regressed.
- Whole loop, serially: **38 suites, 5,426 assertions, 0 FAIL, 0 SKIP, every script exit 0**;
  `tests/baseline.json` re-recorded from that run.

### Fixture changes this session left in the development database

Recorded rather than tidied away, and none of it is in `msms_test`:

- `ui-principal@msms.local`'s password was reset to drive the school screens, as
  `ui-audit@msms.local`'s was in §2am. Neither account exists in the test database.
- School 12797 now has a currency and timezone in `school_settings`, an `exam_subjects` row on exam
  2124, and one submitted mark of 72 against student 11320 — who was moved from `transferred` back to
  `active` so the marks screen had a roll to render. That last one is the only change made *to* the
  fixture rather than *by* the product.
...
verification script. **5,429 checks green, 0 failing, 0 skipped, across thirty-eight scripts**, every
...
a parallel run reports false failures). Per-script assertions, in the order the loop runs them:
    ...
    219, 174, 163, 280, 109, 57, 161, 262, 100, 104, 116, 20
    16, 175, 313, 112, 168, 28, 22, 89, 136, 212, 87, 115, 236, 35  =  5,429

`npm test` wraps that as **5,629 tests, exit 0** — the 5,426 as named cases, five per suite and ten
harness-integrity tests.

*(That array is `tests/baseline.json`, re-recorded from the session-27 run. Session 26's figures were
5,162 with `verify-frontend` at 51; that suite is now 280, and 76 of the difference is this session's
route assertions. Read a per-suite figure from the baseline file and never from this one — `npm test`
checks the **checklist** against the baseline and does not check the progress log at all.)*
...
`src/payments/` does not exist **as a directory**, and that is not the gap it looks like: the plugin
architecture §13.2 requires lives at `src/services/paymentGatewayService.js` (366 lines), with the
registry, the validated adapter contract, `charge()`/`refund()` dispatch, `payment_transactions`
writing on both, and a refusal that names what is missing. It ships **zero adapters on purpose** —
the SRS names no provider, and §35 lists *"Additional payment gateways"* among the things it does not
specify — so shipping one would invent a requirement and would make the *plugin* claim false by
turning the built-in into the assumed shape. `verify-billing.js` covers it.

**5.2 is therefore the one genuinely open row of this phase**: the Anthropic adapter is written and
has never been executed, for want of a key.
...
**5,629 tests, exit 0**, being the 38 suites' 5,429 assertions as named cases plus five per suite and
...
conflict, partial fee payment and pagination. **Rows 6.3 and 6.4 read Completed**, closed by
`verify-security.js`; earlier revisions of this file said otherwise long after it was written.
...
| 4 | ~~**`npm test` fails**~~ **Resolved in session 26.** `tests/setup.js` was referenced by `setupFilesAfterEnv` but absent, and jest resolves that path during config normalization — so the run died with a ValidationError before discovering a single test. | None. `npm test` runs the whole safety net: **5,626 jest tests, exit 0** (session 27; it was 5,358 when this row was written) — 5,426 assertions from the 38 `verify-*.js` suites, each a named test case, plus five per suite and ten harness-integrity tests. | Done — checklist row 6.17. |
...
| 21 | ~~**A limit check and the write it guards are not atomic.**~~ **Resolved in session 28 — all four racing keys, by two different instruments.** **Scoped in session 26, and the scope is narrower and the remedy different from what this row said.** It affects **four** of §11.2's eight limit keys, not eight: `student_limit`, `teacher_limit`, `staff_limit` and `ai_limit`. The other four cannot race for four separate reasons — `admin_limit` has no guard mounted anywhere (its only writer is platform-scoped, and `enforceLimit` exempts platform callers outright); `storage_limit` is never incremented and the code says a guard there "can never fire"; `api_limit`'s counter has no writer, so it is permanently 0; and `file_upload_limit` is PER_REQUEST and reads no shared state, so there is nothing to undercount. It is also **two different bugs**: a headcount race whose window is five to eight round trips plus a transaction, and an `ai_limit` race whose window is an entire LLM provider round trip. No existing constraint serializes either — `subscriptions.school_id` is a **non-unique** index (confirmed against `msms_test`: `Non_unique: 1`), and the `FOR UPDATE` in `allocateStudentId` is taken *after* the count, so it orders two admissions without correcting either. `enforceLimit` (and `assertWithinLimit` called from a service) counts, decides, and then the handler writes — two concurrent requests can both pass a ceiling of N and leave the school at N+1. Found by the session-16 audit of `teachers/`, but it is a property of the guard itself, present since session 4, not of any one module. | Small and self-correcting in practice: the next `syncHeadcount` records the true figure, overage pricing (§11.2) already covers a school that exceeds a soft ceiling, and the window is one request. It is a real hole in a **Fixed** limit on a plan with `allow_overage` false. | **This row's own prescription is not achievable and is corrected here.** It read: "Close it in `enforceLimit` for every limit at once — a locking read inside the same transaction as the write." `enforceLimit` is **express middleware**; the transaction it would have to join is opened later inside the service, or does not exist at all — `teachers.service.js` and `staff.service.js` create with a bare insert. Middleware cannot join a transaction it does not own. The fix has to move into the five service functions that write, or not happen; the unit of consistency is the three headcount modules rather than "every limit". The warning that followed it — do not fix one module in isolation — still stands. **A patch plan exists and is NOT safe to apply as written:** its central design decision (lock the subscription row) is justified by "it is one row per school", which is false, and the lock it proposes carries no `order` and no state filter, so it can lock a row other than the one `governingStateFor()` would choose. `ai_limit` is separately out of reach of that shape entirely — wrapping a provider call in a transaction holds a row lock across the network — and needs reserve-then-refund instead. **A second attempt reworked the mutex and was refused again, though it got closer.** It rejects both rows the first plan considered — the governing subscription and the `usage_records` row — on the ground that **neither is guaranteed to exist**, and measured that a `FOR UPDATE` matching no row serialises nothing, which is the real hazard rather than the non-uniqueness alone. Its answer is to lock the **school row by primary key** as the transaction's first statement; that row always exists, is reachable by the primary key rather than a new index, and the plan checked for a lock cycle and for `paranoid: true` on `schools`. What refuted it was its own proof: one of its named regressions **cannot fail with the defect present** — dropping `{ transaction: t }` from a module's `assertWithinLimit` call was supposed to make two concurrent requests both return 201, and there is no CLS in this project, so an untransacted query does not behave the way the matrix assumed. That regression underwrites five of the seven proposed edits. **So the mutex is probably right and the evidence for it is not yet.** Next attempt: keep the school-row lock, and rebuild the proof matrix against what an untransacted read actually sees. **Closed for the three headcount keys in session 28, on the third attempt's design and a rebuilt proof.** The refutation above was about the *evidence*, not the mutex, so the school-row lock was kept and the proof re-derived by measurement instead of by a matrix. `usageService.reserveHeadcount(schoolId, limitKey, increment, transaction)` takes a `SELECT … FOR UPDATE` on `schools` by primary key and then calls `assertWithinLimit` inside the caller's transaction — `transaction` was threaded through `countHeadcount` → `getUsage` → `checkLimit` → `assertWithinLimit` to make that possible. **The order of the two statements is load-bearing and is the part the earlier plans did not state:** under InnoDB REPEATABLE READ a locking read is a *current* read and does **not** establish the transaction's consistent snapshot — the first plain `SELECT` does — so taking the lock first means the snapshot is established after the lock is granted, and therefore after any competing transaction has committed, and the count sees its row. Counting first would fix the snapshot before the lock and miss exactly the write this exists to see. That is also why the reservation must be the caller's transaction's **first** statement, which it is in all three services. `teachers.service.js` and `staff.service.js` had no transaction at all — the bare insert this row already named — so both now open one. In those two the reservation is guarded by `payload.is_active !== undefined ? payload.is_active : true`, the same predicate `HEADCOUNT_SOURCES` counts by, because a reservation stricter than the count is its own bug: an unguarded version regressed `verify-teachers.js`, which asserts that an inactive teacher is admissible at the ceiling. **Measured before and after, three modules, eight concurrent creates against a ceiling of 1:** before, 8 admitted and 8 rows, every run, 3 of 3 racing; after, 1 admitted and 1 row with the other seven `PLAN_LIMIT_EXCEEDED`, every run. **`scripts/verify-concurrency.js` is that measurement made permanent** — 11 assertions, the 39th suite, run 15 times consecutively at 11/0 and proved against the defect by commenting the three `reserveHeadcount()` calls out, which returns it to 8 rows on all three modules and 9 FAILED. **`ai_limit` was closed in the same session, by reserve-then-refund rather than by a lock — and it needed no new table.** Its critical section is an entire LLM provider round trip, so the remedy above would hold a `schools` row lock across a call to a third party: every other write for that school blocked for as long as the provider takes, and a provider outage becoming a database pile-up. `usageService.reserveUsage()` takes the allowance **before** the driver instead, as one conditional statement — `UPDATE usage_records SET used_value = used_value + n WHERE id = ? AND used_value + n <= allowance` — whose row lock lives and dies inside that statement, so two requests are serialised against each other and neither waits on a network. **Zero rows affected is the refusal**, and there is no read-then-write window in which the answer could go stale. `releaseUsage()` is the refund, and is `recordUsage()` with a negative delta, a path that already existed for storage returned by a deletion and already floors at zero; `ai.service.generate()` calls it on both paths that produce nothing — the provider failing and the write failing — which keeps the promise that file's header already made. **This is why no 65th table was needed**: the counter *is* the reservation, so §29's fixed schema and §35 are untouched. Two cases increment unconditionally and are read from the resolved snapshot rather than from the denormalised `usage_records.allowed_value`: **unlimited**, and **overage allowed**, since §11.2 makes exceeding a soft limit a billing arrangement rather than a refusal and a reservation that refused would enforce a rule the plan does not have. **Measured**: eight concurrent generates against `ai_limit: 1`, each on its own bank because the stage machine already serialises two generates on one — before, 8 admitted and the counter at 8; after, one admitted, seven `PLAN_LIMIT_EXCEEDED`, counter at exactly 1. The refund is asserted separately by replacing the driver's `generate` for one call, and given a positive beside it: the same bank charges one when the provider works. Eight more assertions in `scripts/verify-concurrency.js` (19 now), five rewritten in `verify-ai.js`, which had pinned the old arrangement at the source. Six failures on the deliberate regression. **Original note:** Do not fix it in one module: the inconsistency would be worse than the gap. |
...
| 32 | ~~**A student photo can be stored and can never be looked at.**~~ **Resolved in session 28.** `POST /students/:id/photo` is the only writer `students.photo_path` has, and **nothing reads it**: `present()` deletes the column and returns `has_photo` in its place, `sendStoredFile` has exactly three callers (assignments, homework, payments) and students is not one, and a grep of `backend/src` finds no other reader — the ID-card document included, which builds its payload from the record rather than the file. So the upload sets a flag and writes bytes to disk that no screen can display. | FR-STUDENT-001 names a photo and the data is captured, so nothing is lost; but a school that uploads one has no way to check it, and the ID card it would belong on does not use it. | Mount a `GET /students/:id/photo` alongside the writer, using `sendStoredFile` exactly as `payments.controller.js` does for its screenshot — the pattern, the permission shape and the inline/attachment handling all already exist. That is backend surface rather than a specification question, which is why this is a defect and not a `real-blocked` finding. Found on 2026-09-09 while wiring the upload; the screen states the absence rather than implying a viewer. **Done, exactly as prescribed.** `GET /students/:id/photo` is mounted beside its writer, on `students.view` rather than the writer's `students.manage` — looking at a record's photo is looking at the record — and loads through the same tenant-scoped `findById()` that `GET /:id` uses, so the boundary deciding which students a caller may see decides which photos they may see, with no second rule to maintain. Served `inline`, named by the school's own `student_id` rather than the primary key, and a student with no photo is a **404 with a message** rather than an empty 200, since `has_photo` already tells a caller whether to ask. **The screen shows it**: the panel's honest "there is no viewer" notice is replaced by the image, fetched as a blob through the authenticated client because the token only travels in an `Authorization` header and a browser fetching an `<img src>` sends none — the shape `super-admin/payments` established for FR-BILL-004's screenshot. Eleven assertions in `verify-students.js` (147 now, was 136) and two in `verify-frontend.js`. **One of them could not fail when first written and was rebuilt:** the cross-school read was aimed at a student with no photo, so the route's own "no photo" 404 answered it and replacing the tenant-scoped finder with a bare `findByPk` left it green — proved by exactly that regression, then re-aimed at a student in another school who **has** one, with the positive beside it. Three further deliberate regressions all fired. Driven in a browser afterwards: the record's stored photo decoded at its natural 96×120 from a `blob:` URL. |
| 33 | **Runtime logs and an uploaded file were committed to git, and they are still in the history.** `.gitignore` named runtime storage under §26 and looked right; it was not. A pattern with a slash anywhere but at the end is **anchored to the file's own directory**, so `storage/logs/` matched `<repo>/storage/logs/` — a path that does not exist — and never `backend/storage/logs/`, which is the one that does. Eleven files were tracked: ten rotated log archives and one uploaded student photo. Found in session 28 from a `git commit` summary that named three `.log.gz` files. | `.env` was never among them and is separately asserted, so no deployment credential leaked. The logs are not inert, though: `.env` sets `MAIL_DRIVER=log`, and `mailService.js` says in its own header that this renders the outgoing message into the application log — *"a reset link appears in `storage/logs`"* — so every development password-reset mail, link included, is written there. They also carry user ids, school ids, request paths and client IPs. The repository is local and was initialised on 2026-09-09, so the exposure is to whoever receives this repository, not to the internet. | **Half fixed, and the half that is left is deliberate.** The patterns are now `**/`-prefixed and the eleven files are out of the index (`git rm --cached`, so they stay on disk). `verify-deploy.js` part 7 asserts **both** halves — the four patterns, and `git ls-files` returning nothing under any `storage/` runtime directory — because a pattern only governs what is not already tracked, and fixing the pattern alone leaves exactly the state this was found in. Both proved by deliberate regression. **What is NOT done: the history still contains them.** Removing them needs a rewrite of every commit that touched them, which changes every hash after it, and that is a destructive operation on someone else's repository — it is the owner's call, not mine. If this repository is ever pushed anywhere, do that first: `git filter-repo --path backend/storage --invert-paths`, then force-push, and invalidate any password-reset token that is still inside its 60-minute window. |
...
| `students/students.routes.js` | 159 | Nine routes — `GET /:id/photo` joined its writer in session 28 (Known Issues #32), on `students.view` and declared above `GET /:id` so the literal segment can never be swallowed. `requireModule(MODULES.STUDENTS)` router-level, `enforceLimit(LIMITS.STUDENT_LIMIT)` on `POST /`, and the SRS's own actor split — `students.manage` for admission and profile, `students.progression` for the three lifecycle routes |
| `students/students.controller.js` | 140 | Nine handlers. `photo` is the reader Known Issues #32 wanted: the same tenant-scoped `findById()` `show` uses, `sendStoredFile` inline, named by the school's own `student_id` rather than the primary key, and a 404 with a message for a student who has none |
...
| `ai/ai.service.js` | 676 | The stage machine as a frozen `TRANSITIONS` table, the driver calls, and **the project's only production caller of `usageService.reserveUsage`**. It read `recordUsage` after the commit until Known Issues #21 — correct about not charging for a failure, silent about two requests at the cap both passing; the allowance is now taken **before** the driver and released on both paths that produce nothing, which keeps the same promise under concurrency. Both writes to `questions` go through loaded instances, because a bulk `Model.update()` trips the MCQ validator against a partial row |
...
| `addons/addons.service.js` | 495 | `list`, `findById`, `update`, `setActive`, `setPrices` (deletes the unreferenced, retires the referenced — the same rule as `plans.setPrices`), plus `readiness`, `scopeFor`, `detailInclude`, `planIdFor`, `unitFor`, `pricesInUse`, `assertPlansExist`. `detailInclude` takes the school's plan id since Known Issues #17 and filters `addon_prices` to the unrestricted rows plus that plan's, which is the write path's own refusal inverted into a filter; `planIdFor` resolves it so the helper can stay synchronous. **No `create` and no `destroy`, deliberately.** The only write module in the subscription area that calls no `entitlementService.invalidate*`; the header explains that `resolveSchool()` never joins `addons` because `subscription_addons` copies the effect at purchase, so there is no cached value to stale |
...
| `usageService.js` | 870 | `getUsage`, `getUsageSummary`, `checkLimit`, `assertWithinLimit`, `reserveHeadcount`, `reserveUsage`, `releaseUsage`, `checkPerRequestLimit`, `recordUsage`, `syncHeadcount`, `syncAllHeadcounts`, `countHeadcount`, `measurementFor`, `periodFor`, `MEASUREMENT`, `PER_REQUEST_LIMITS`, `HEADCOUNT_SOURCES`. `reserveHeadcount` is the Known Issues #21 fix: it locks the `schools` row by primary key and then counts **inside the caller's transaction**, which is why `transaction` is now threaded through `countHeadcount` → `getUsage` → `checkLimit` → `assertWithinLimit`. It refuses a non-headcount key and refuses to run without a transaction, both as programming errors rather than as runtime conditions. `reserveUsage` is the same fix for the accumulating keys, where a lock is the wrong instrument: it takes the allowance ahead of the work in one conditional `UPDATE` whose lock lives and dies inside that statement, so nothing is held across an LLM provider call — zero rows affected is the refusal. `releaseUsage` is its refund, and is `recordUsage` with a negative delta so the floor-at-zero rule stays in one place |
...
| `verify-concurrency.js` | 435 | 19 checks on Known Issues #21 — the only suite in the loop that runs anything in parallel. Builds its **own** school on a plan whose three headcount ceilings are 1, because sharing a fixture would let eight parallel writes interleave with another suite's assertions, then fires eight concurrent creates at each of students, teachers and staff through the composition the route uses (`assertWithinLimit`, then the service) and reads the table back. Supplies `student_id` and `roll_number` so `allocateStudentId` and `allocateRollNumber` are skipped: their own `FOR UPDATE` reads serialise concurrent admissions as a side effect and **mask** the race being measured, which was found while reproducing it. Asserts the row count rather than the success count, and separately that the seven refusals are `PLAN_LIMIT_EXCEEDED` and nothing else, so a fix that merely made concurrent creates fail some other way cannot pass. Also asserts the fix is not too **strict** — an inactive teacher is still admitted at the ceiling. The fourth racing key, `ai_limit`, is measured here too and fixed by a different instrument: eight concurrent generates on **eight banks**, because the workflow stage machine already refuses a second generate on one, so a single-bank fixture would measure that refusal and never touch the limit. Its refund is asserted by replacing the driver's `generate` for one call, with the positive beside it — the same bank charges one when the provider works — since "the counter did not move" also describes a reservation that never incremented. Pins `AI_DRIVER=mock` itself rather than inheriting it, so the loop cannot make a network call because of a local `.env`. Ends by checking its own fixture is gone |

### `backend/tests/` — the jest fold (row 6.17)

| File | Purpose |
|---|---|
| `globalSetup.js` | Runs all 39 suites serially as child processes before jest evaluates any test file, then writes the parsed results. Proves the target with `SELECT DATABASE()` — `DB_NAME` states an intention, `SELECT DATABASE()` states a fact — and closes its own pool before the loop starts, because a pool left open is one the suites cannot use, and connection exhaustion reaches a suite as "database unreachable", which 19 of them answer by skipping and exiting 0. |
| `verify.test.js` | Turns the run into named jest cases: one per assertion, carrying the label the suite printed, plus **five per-suite tests** (exited 0, did not time out, printed no FAIL line, ran its database half, ran all N of its assertions) and **six harness-integrity tests**. That is the arithmetic behind the total: 5,426 + (38 × 5) + **10** = **5,626**, measured from the session-27 run's own output. The harness-integrity count was **6** when this sentence was written and is now ten — it is counted from the run rather than carried, because it is the one term here that grows without any suite changing. Reads the results **synchronously at module scope** — jest builds its test tree by executing the module body, so a `test()` registered from an async callback or a `beforeAll` is never collected, and an `await` here would yield a file with zero tests that reads as "nothing to check". |
...
337. Closed the **rest** of the uncalled-write-route queue (§2an) — nine clusters and eight single
     routes, taking the measurement from 75 of 150 to **149 of 150**. The one left is
     `POST /auth/refresh`, a false positive: `apiClient` reaches it by raw `fetch` because it is the
     refresh mechanism.
338. Resolved the *needs a decision* column rather than deferring it, and the resolution is one
     sentence: **§33 fixes two screen lists, and a screen list is not the requirement.** School
     settings, academic sessions, assignments, notifications, taxes, quotations and the AI workflow
     are all stated requirements with built, verified APIs and no way in; each is now reached from a
     dashboard or a sibling screen rather than from a nav the suite asserts against §33. The platform
     Settings screen stays the counter-example and still has no form.
339. Found the `api.<method>(` blind spot **four more times**, in screens that had been calling their
     routes correctly all along — payment approve/reject, add-on activate/deactivate and both finance
     ledgers. Eleven working routes reported as uncalled by the check written to catch exactly that.
340. Built `components/editDialog.tsx` for the seven screens that needed the same `PATCH /:id` at
     once, on the split `useRowAction` states: the dialog owns the state machine, the caller owns the
     words.
341. **Every list picker in the new work asked for more rows than the API allows.**
     `PAGINATION.MAX_LIMIT` is 100 and four screens asked for 200 or 500, which is a 422 rendering as
     an empty select — found by driving the exam papers screen, whose subject picker was empty for a
     school that has one. `verify-frontend.js` now refuses it, reading the ceiling from the backend.
342. Had three more defects caught by the suites on new code: a screen reading `extra_permissions`,
     which the API strips and always returns undefined; a create-screen census that had to move 18 →
     19; and three form-state interfaces typing a money column as a string. All fixed in the code.
343. **Shipped an assertion that could never fire, from the heredoc-backspace failure this project
     has now hit three times** — caught by `npm test`'s stray-control-character rule, repaired with
     an editor, and then proved by deliberate regression.
344. Widened `verify-security.js`'s link rule after it refused a guard that was **stronger** than the
     one it looked for: the notification centre admits internal paths only, where the rule wanted an
     `^https?:` allowlist that an external URL would pass. Regressed to confirm it still fires.
345. Drove the new work in a browser as the school principal: school settings saved and read back
     from the database; a paper added, a mark of 72 entered and submitted, after which the service
     derived `outcome: pass` and locked the paper. `grade_name` stayed null for want of a grade
     band — the two new screens agreeing rather than either being wrong.
346. Re-ran the whole loop: **38 suites, 5,426 assertions, 0 FAIL, 0 SKIP, every script exit 0**;
     `npm test` **5,626 tests, exit 0** after the two harness failures above were fixed.
347. **Closed the headcount half of Known Issues #21**, the one open row with real consequences.
     Reproduced it first rather than fixing it first: eight concurrent creates against a school whose
     `student_limit`, `teacher_limit` and `staff_limit` were each 1 admitted **eight, and wrote eight
     rows, on all three modules, every run**.
348. Kept the third attempt's mutex — lock the `schools` row by primary key — since §5's refusal of it
     was about its *proof*, not its design, and rebuilt the proof by measurement.
     `usageService.reserveHeadcount()` takes the lock and then counts inside the caller's transaction;
     `transaction` was threaded through `countHeadcount` → `getUsage` → `checkLimit` →
     `assertWithinLimit` so that it can.
349. **The order of the two statements is the part every earlier plan left out.** Under InnoDB
     REPEATABLE READ a locking read is a *current* read and does **not** establish the transaction's
     consistent snapshot — the first plain `SELECT` does. Lock first and the snapshot is taken after
     the lock is granted, so the count sees the competing transaction's committed row; count first and
     the snapshot is fixed before the lock and misses exactly the write the lock exists to catch. It
     therefore has to be the caller's transaction's **first** statement, and is, in all three services.
350. `teachers.service.js` and `staff.service.js` had **no transaction at all** — the bare insert §5
     already named — so both now open one.
351. **Made the reservation exactly as strict as the count, after being too strict.** Guarding
     unconditionally regressed `verify-teachers.js`, which asserts an inactive teacher is admissible at
     the ceiling; the guard is now the same predicate `HEADCOUNT_SOURCES` counts by. A fix that is too
     strict is still a defect, and this one was caught by an existing suite rather than by review.
352. **Turned the reproduction into `scripts/verify-concurrency.js`, the 39th suite** — 11 assertions,
     and the only one in the loop that runs anything in parallel. It asserts the **row count** rather
     than the success count, and separately that the seven refusals are `PLAN_LIMIT_EXCEEDED` and
     nothing else, so a "fix" that merely made concurrent creates fail some other way cannot pass it.
353. Ran the new suite **15 consecutive times: 11 PASS / 0 FAIL every time**, per the standing rule that
     one green run of a new suite proves nothing — which matters more here than usual, a suite whose
     subject is a race being the obvious candidate for a flaky one.
354. **Proved it against the defect it exists for**: commenting the three `reserveHeadcount()` calls out
     returns it to 8 rows against a ceiling of 1 on all three modules, **9 FAILED**. Restored and
     re-confirmed green.
355. Re-measured `scripts/`'s own lint debt while updating the count of suites and found the recorded
     figure stale by one in **both** places that cite it — 45 against a real **46**, and the new suite
     lints clean, so it contributed none of them. Both now record the full six-rule breakdown rather
     than "4 others", the four unnamed rules being exactly the ones that could drift unseen.
356. Re-recorded `tests/baseline.json` and re-ran the whole loop: **39 suites, 5,440 assertions, 0 FAIL,
     0 SKIP, every script exit 0**; `npm test` **5,645 tests, exit 0**.
357. **Closed Known Issues #32 — a student photo could be stored and never looked at.** `photo_path`
     had exactly one writer and no reader anywhere in `backend/src`. `GET /students/:id/photo` is now
     mounted beside its writer, on `students.view` rather than the writer's `students.manage`, loading
     through the same tenant-scoped `findById()` that `GET /:id` uses so the boundary is one rule and
     not two.
358. **An assertion I wrote for it could not fail, and the regression is what said so.** The
     cross-school read was aimed at a student with **no** photo, so the route's own "no photo" 404
     answered it: replacing the tenant-scoped finder with a bare `findByPk` left it green. Re-aimed at
     a student in another school who has one, with the positive beside it — the same photo *is*
     readable by its own school — and it now fires. Three other deliberate regressions fired first try.
359. Gave the screen the viewer, and deleted the honest notice that had said there was none. The bytes
     arrive as a **blob through the authenticated client**, because the token only ever travels in an
     `Authorization` header and a browser fetching an `<img src>` sends none — the shape
     `super-admin/payments` established for FR-BILL-004. Drove it as the principal: the stored photo
     decoded at its natural 96×120 from a `blob:` URL.
360. **Closed Known Issues #17 — a school could see add-on prices restricted to plans it is not on.**
     `detailInclude()` filtered on `is_active` alone. It now also filters `plan_id IS NULL OR plan_id =
     <the school's>`, resolved by a new `planIdFor(tenant)` in the two callers, both already async — so
     the helper stayed synchronous and nothing outside the file changed signature, which is what the
     row's session-26 correction predicted and its original excuse denied.
361. **The filter is the write path's refusal, inverted rather than invented.** `purchaseAddon()`
     refuses when `price.plan_id && price.plan_id !== subscription.plan_id`; a read filter stricter or
     looser than that would be a new rule, and §35 has none to offer. Three prices on one add-on —
     unrestricted, this plan's, another plan's — because two cannot separate "hides too much" from
     "hides nothing", and asserted on **both** call sites, a fix to `findById` alone leaving the
     listing to leak. Regressed both ways and both fired.
362. Corrected a comment in `verify-frontend.js` that said nothing was found suppressing `photo_path`.
     `students.service.present()` does suppress it; what keeps it out of `SUPPRESSED_PATHS` is that
     `parents`, `staff` and `teachers` return the column as it stands, permanently null for want of a
     writer. The rule is unchanged; the reason recorded for it now matches the code.
363. Re-recorded the baseline and re-ran the whole loop: **39 suites, 5,460 assertions, 0 FAIL, 0 SKIP,
     every script exit 0**; `npm test` **5,665 tests, exit 0**. The harness's checklist-versus-baseline
     test failed first and named all three stale figures, which is the check working.
364. **Closed the `ai_limit` half of Known Issues #21 too, so the whole row is now shut.** Every earlier
     plan for it — including my own, twice in this session — said it needed a reservation *row*, which
     §29's fixed 64 tables and §35 forbid. It does not: the **counter is** the reservation.
365. `usageService.reserveUsage()` takes the allowance ahead of the provider as one conditional
     statement — `UPDATE usage_records SET used_value = used_value + n WHERE id = ? AND used_value + n
     <= allowance` — whose row lock lives and dies inside that statement, so two requests serialise
     against each other and neither waits on a network. **Zero rows affected is the refusal**, with no
     read-then-write window for the answer to go stale in. `releaseUsage()` is the refund and is
     `recordUsage()` with a negative delta, which already existed for storage returned by a deletion.
366. **Two cases increment unconditionally, and both are read from the snapshot rather than from
     `usage_records.allowed_value`** — which is a denormalised copy a mid-period plan change leaves
     stale. Unlimited, because there is no ceiling; and **overage allowed**, because §11.2 makes
     exceeding a soft limit a billing arrangement and a reservation that refused would enforce a rule
     the plan does not have.
367. Narrowed the settled figure back to the four fields `recordUsage` returned, so the generate
     response is byte-for-byte what it was. `getUsage` answers with eleven more, none of which was ever
     in that body — widening an API as a side effect of an internal fix is how a contract changes
     without a decision.
368. Rewrote five source-level assertions in `verify-ai.js` that had pinned the **old** arrangement —
     "exactly one `recordUsage`, after the transaction, so a failed generation is never charged". That
     was right about the refund and silent about the race. They now pin the order (`reserveUsage`
     *before* `aiDriver.generate`), both refunds, and that `recordUsage` is not called a second time,
     which is the double-charge this shape invites.
369. Extended `scripts/verify-concurrency.js` to the fourth key: eight concurrent generates against
     `ai_limit: 1`, each on **its own bank**, because the workflow stage machine already refuses a
     second generate on one bank — a single-bank fixture would measure that refusal and report a pass
     having never touched the limit. Before the fix, 8 admitted and the counter at 8; after, one
     admitted and the counter at exactly 1.
370. Asserted the refund with a positive beside it — a provider failure costs nothing, **and** the same
     bank charges one when the provider works — because "the counter did not move" also describes a
     reservation that never incremented at all.
371. Pinned `AI_DRIVER=mock` in the suite rather than inheriting it, so the verification loop cannot
     make a network call because of what a local `.env` happened to say.
372. Ran the extended suite **15 consecutive times: 19 PASS / 0 FAIL, exit 0 every time**, and regressed
...
388. **Found Known Issues #33 in a `git commit` summary, of all places** — it named three `.log.gz`
     files, which nothing in `backend/storage/` should ever have been in a position to add.
     `.gitignore` names runtime storage under §26 and looked right: a pattern with a slash anywhere
     but at the end is **anchored to the file's own directory**, so `storage/logs/` matched
     `<repo>/storage/logs/`, a path that does not exist, and never `backend/storage/logs/`. Eleven
     files were tracked — ten rotated log archives and an uploaded student photo.
389. The logs are not inert. `.env` sets `MAIL_DRIVER=log` and `mailService.js` says in its own header
     that this renders the outgoing message into the application log — *"a reset link appears in
     `storage/logs`"* — so every development password-reset mail is in there, link included, along
     with user ids, school ids, request paths and client IPs. **`.env` itself was never tracked**, and
     that is now separately asserted rather than assumed.
390. Fixed the patterns with `**/` and took the eleven out of the index with `git rm --cached`, which
     leaves them on disk. `verify-deploy.js` gained a **part 7** asserting both halves — the four
     patterns, and `git ls-files` returning nothing under any `storage/` runtime directory — because a
     pattern only governs what is not already tracked, and fixing the pattern alone leaves exactly the
     state this was found in. Neither assertion alone would have caught it; both regressed.
391. **Did not rewrite the history, and that is a decision rather than an omission.** The files are
     still in every commit that touched them, and removing them changes every hash after it. That is
     destructive and it is the repository owner's call; #33 records the exact command and says to run
     it before this repository is pushed anywhere.
392. Final run: **39 suites, 5,484 assertions, 0 FAIL, 0 SKIP, every script exit 0**; `npm test`
     **5,689 tests, exit 0**; `eslint src tests --max-warnings 0` exit 0; `eslint .` in `frontend/`
     exit 0; `tsc --noEmit` exit 0; `next build` compiled clean.
...
and the one exception is a false positive: `POST /auth/refresh` is reached by a raw `fetch` in
`apiClient.ts:302` because it *is* the refresh mechanism and cannot use the client that depends on it.

Reproduce that before trusting it. The script is short — `buildDocument(createApp())` against the
`api.<method>('…')` literals in `frontend/src` — and the measurement errs in **both** directions, as
...
- **5.2 — the Anthropic adapter's round trip.** Everything but the network hop is exercised and
  ...
  have; a stub agreeing with itself is not evidence about the service. When it is attempted, **start
...
- ~~**`src/payments/` — the plugin registry and five providers.**~~ **This entry was wrong, and it was
  wrong when I wrote it earlier in this same session — from the phrasing §4 had carried rather than
  from the code.** The architecture §13.2 requires exists at `src/services/paymentGatewayService.js`,
  is 366 lines, is wired into `payments.service.js` for both charge and refund, and is covered by
  `verify-billing.js`. It ships **zero adapters deliberately**: the SRS names no provider and §35
  lists *"Additional payment gateways"* among what it does not specify, so an adapter would invent a
  requirement **and** falsify the plugin claim by making the built-in the assumed shape. "Five
  providers" was never in the source. Adding a real one is `register({ key, label, charge, refund })`
  plus an env var, with no change to `payments.service.js` — which is the test of whether the
  architecture is plugin-based, and it passes.

  **This is the second time the same false claim has been made and corrected.** §5a already records
  checklist row 5.1 moving from *Pending, "`src/payments/` does not exist"* to Completed, with the
  note that the path came from `docs/ARCHITECTURE.md` and predates the code by two days. The claim
  came back because §4 and §7 still carried the old phrasing, and I copied it from there rather than
  from the code — the exact failure §8 warns about. **A path is not a capability**: before recording
  that something does not exist, grep for what it would do, not for where a document said it would
  live.
- ~~**Rows 6.3 and 6.4.**~~ **Also wrong, and from the same cause as the payments entry above:** both
  rows read **Completed** in the checklist and have since `verify-security.js` was written. The
  sentence about coverage being "spread across seven suites" described the state *before* that suite
  existed, and it survived in §7 because each session rewrote the brief around it rather than
  re-deriving it. **Measured**: exactly three checklist rows are not Completed — 3.S.1, 5.2 and 7.13,
  all three named above and none of them buildable here.
- ~~**The frontend has no lint at all.**~~ **Done** (§2an): `eslint.config.mjs`, the script repointed
  at `eslint .`, seventeen real findings fixed, and `verify-deploy.js` asserting it the way it
  already asserted the backend's. **What is left of it is one rule**:
  `react-hooks/set-state-in-effect` is off, because its 66 findings are two patterns — a fetch effect
  setting `loading` before awaiting, and a form re-seeding when its row changes — whose fixes are
  architectural rather than local. Turning it back on is the acceptance test for whichever fetching
  architecture replaces the current one; that decision is the work, not the suppression.
- **§5's Known Issues register is now down to rows that cannot be closed by writing code here.**
  Sessions 28 closed **#21** (all four racing keys), **#17** and **#32**. What is open is **#2** (MySQL
  does not survive this environment — environmental), **#11** (the seeded Super Admin password is still
  the `.env.example` value locally; the seeder already hard-refuses it under `NODE_ENV=production`, so
  what is left is setting a real value at deploy time), **#15** (a note, not a defect), **#18** — whose
  remedy is explicitly *do not fix*, an implicit price selection being a guess at an `addon_prices`
  default the SRS does not name — **#19** (`wallet_balance`: §13.2 lists "Wallet" among five payment
  ...
  practice, and §6's harness enforces it) and **#31**, which needs a house-style decision about marking
  verbatim source text, not a patch. Each of the five open behavioural rows names what it is waiting on.
...
validated by the tool that will consume it; `verify-deploy.js` checks that each agrees with the
application, which is a different claim and a weaker one. The likeliest thing to break at the first
real deploy is the backup user's privileges — `mysqldump --single-transaction --routines --triggers`
needs the global `PROCESS` grant.

Standing practice, with what this session added:

- **Run the loop serially at the *start* of a session** (Known Issues #25), and **re-record
  `tests/baseline.json`** when a suite grows: `npm test` asserts each suite's exact count, so a grown
  suite fails the harness until the baseline is deliberately re-recorded.
- **Read a per-suite figure from `tests/baseline.json`, never from prose.** `npm test` checks the
  *checklist* against the baseline and does not check `IMPLEMENTATION_PROGRESS.md` at all. Session 27
  found nineteen of §1's thirty-eight cited counts stale, one by 154, every one of them green
  throughout.
- **Never author a regex through a heredoc or a non-raw Python string.** This project has now shipped
  a backspace byte into a regex **three times**, and twice the result was an assertion that passed
  while proving nothing. `npm test`'s stray-control-character rule catches it; use an editor.
- **A sweeping regex over a document with two tables will edit both.** §1's assertion counts and §6's
  line counts share a row shape. Slice the document at the section heading, print every change, and
  read the printout.
- **A screen may be looser than the API; it may not be tighter.** §2am shipped a select that disabled
  every add-on the API would have sold. Before disabling a control, find the line in the service that
  refuses it.
- **Check a list read against `PAGINATION.MAX_LIMIT`.** It is 100, and a larger `limit` is a 422 that
  renders as an empty picker rather than as an error anyone sees. Four screens carried it at once;
  `verify-frontend.js` now refuses it.
- **Restart MariaDB as a *tracked* background process** —
  `mysqld --defaults-file=C:/xampp/mysql/bin/my.ini --standalone` — and restart both dev servers
  afterwards, because they die with it.
...
- **Aim a tenancy assertion at a row that would otherwise be returned.** Session 28's cross-school
  photo read was pointed at a student with no photo, so the route's own 404 answered it and the tenancy
  rule was never exercised — green with the finder replaced by a bare `findByPk`. Give the negative a
  positive beside it: the same row *is* readable by the caller who owns it.
- **A concurrency fix is only as good as the thing that reproduces it.** Reproduce the race *first*, in
  the shape a request takes, and keep that reproduction as a suite — nothing else in a serial loop can
  fail when the guard breaks again. And watch what the fixture serialises by accident:
  `allocateStudentId`'s own `FOR UPDATE` masked this race entirely until both ids were supplied.
- **Under REPEATABLE READ, the first plain `SELECT` fixes the snapshot, not the lock.** So a
  lock-then-count critical section must take the lock **before** anything reads, and must be the
  transaction's first statement. A plain read placed above it reintroduces the defect and changes
  nothing visible.
...
- **A `.gitignore` pattern with a slash in the middle is anchored to that file's directory.**
  `storage/logs/` in the root `.gitignore` never matched `backend/storage/logs/`, and eleven runtime
  files were tracked for it. Use `**/` for anything that lives under a workspace directory, and check
  `git ls-files`, not just the pattern — an ignore rule does not untrack what is already tracked.
