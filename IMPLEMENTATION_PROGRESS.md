...
process.
**5,460 checks pass, 0 fail, 0 skip, across thirty-nine scripts**, every script exit 0, measured as one
...
counting output lines undercounts the loop by 21 — a serial run prints 5,439 `PASS` lines for **5,460**
assertions.
...
table above is the session-28 figure: **5,460 / 0 FAIL / 0 SKIP / every script exit 0**, from a serial run.
...
| `usageService.js` | 712 | `getUsage`, `getUsageSummary`, `checkLimit`, `assertWithinLimit`, `reserveHeadcount`, `checkPerRequestLimit`, `recordUsage`, `syncHeadcount`, `syncAllHeadcounts`, `countHeadcount`, `measurementFor`, `periodFor`, `MEASUREMENT`, `PER_REQUEST_LIMITS`, `HEADCOUNT_SOURCES`. `reserveHeadcount` is the Known Issues #21 fix: it locks the `schools` row by primary key and then counts **inside the caller's transaction**, which is why `transaction` is now threaded through `countHeadcount` → `getUsage` → `checkLimit` → `assertWithinLimit`. It refuses a non-headcount key and refuses to run without a transaction, both as programming errors rather than as runtime conditions |
...
- **The open rows of §5's Known Issues register.** #17 and #32 were closed in session 28 as well, so
  what is open is #2 (MySQL does not survive this environment — environmental), #11 (the seeded Super
  Admin password is still the `.env.example` value locally; the seeder hard-refuses it in production),
  #15 (a note, not a defect), #18 (whose remedy is *do not fix* — an implicit price selection would
  guess at an `addon_prices` default the SRS does not name), #19 (`wallet_balance`, blocked on §36
  triage), #25, #31 and the `ai_limit` half of #21. #21 — the one with real consequences — is now
  **three-quarters closed**: `student_limit`, `teacher_limit` and `staff_limit` are atomic and
  `scripts/verify-concurrency.js` holds them. **What is left of it is `ai_limit`**, and it is left on
  purpose: its critical section is an entire LLM provider round trip, and the remedy used for the other
  three would hold a row lock across a call to a third party — a worse failure than the one it fixes.
  It needs **reserve-then-refund**: write a reservation row before the provider call, convert it on
  success, release it on failure or timeout, and count reservations alongside usage. That is a schema
  change and a sweep for orphaned reservations, which is why it is not bundled with the headcount fix.
  When it is attempted, `scripts/verify-concurrency.js` is the place for its proof — the fixture and
  the parallel harness already exist there, and `AI_DRIVER=mock` makes the provider call deterministic.
